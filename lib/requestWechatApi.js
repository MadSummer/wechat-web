 ...
 * @Last Modified time: 2017-12-07 18:22:17
 */
...
const api = {
  ...
  getMsgMedia: (param, msg, type) => {
    let opt =
      type == 'images' ?
        'getMsgImgOpt' : type == 'video' ?
          'getMsgVideoOpt' : 'getMsgFileOpt';
    let filename = msg.FileName
      || (msg.MsgId + (type == 'images' ? '.jpg' : '.flv'));
    let id = type == 'file' ? msg.MediaId : msg.MsgId;
    return new Promise((onFullfilled, onRejected) => {
      let file = path.resolve(__dirname, `../file/${type}/`, filename);
      rp(getRequestOpt[opt](param, id))
