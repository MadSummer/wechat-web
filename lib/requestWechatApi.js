 ...
 * @Last Modified time: 2017-12-16 12:36:38
 */
...
const api = {
  ...
  getMsgMedia: (param, msg, type) => {
    let tmp = {
      [mediaType.image.name]: 'getMsgImgOpt',
      [mediaType.video.name]: 'getMsgVideoOpt',
      [mediaType.file.name]: 'getMsgFileOpt',
      [mediaType.voice.name]:'getMsgVoiceOpt'
    }
    let opt = tmp[type];
    let filename = (msg.FileName || msg.MsgId) + '.' + mediaType[type].suffix;
    return new Promise((onFullfilled, onRejected) => {
      let dir = path.resolve(__dirname, `../file/${type}/`);
      let file = path.resolve(dir,'./' + filename);
      fse.ensureDirSync(dir);
      rp(getRequestOpt[opt](param, msg))
