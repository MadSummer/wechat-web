 ...
 * @Last Modified time: 2017-12-05 19:56:46
 */
...
const api = {
  ...
  login: async result => {
    ...
    // 第二步，获取扫码结果
    if (result.step === 'qr') {
      let res = await api.QRcodeScanResult(result.uuid);
      if (res.code == 201) {
        return {
          step: 'waiting'
        }
      ...
      if (res.code == 200) {
        let res = await rp(getRequestOpt.initWebWXOpt(info)).then(res => {
          return res;
        });
        let info = await
          rp(getRequestOpt.redicetURLOpt(res.redirect_uri))
            .then(res => {
              saveCookie(res);
