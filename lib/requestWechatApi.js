 ...
 * @Last Modified time: 2017-12-06 18:28:00
 */
...
const api = {
  ...
  pushLogin: uin => {
    return rp(getRequestOpt.pushLogin(uin)).then(res => {
      ...
      let cookie = jar.getCookieString(getRequestOpt.pushLogin(uin).uri)
  ...
  login: async progress => {
    ...
    if (!progress) {
      ...
      if (cookieUin && !isCookieExpired(cookieUin)) {
        ...
        if (cookieDataTicket && !isCookieExpired(cookieDataTicket) && false) {
          // login
          let info = {
